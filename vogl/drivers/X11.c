...
X11_init(void)
{
	...
	if (getenv("USEOWNCMAP")) {
		colormap = XCreateColormap(display, winder, DefaultVisual(display, screen), AllocAll);
                privatecmap = 1;
	} else
		colormap = DefaultColormap(display, screen);
...
*/
	...
	if (vdevice.wintitle)
		...
		strcpy(name, "Vogl window");
#else
	strcpy(name, "Vogle window");
#endif
...
X11_checkkey(void)
{
	...
	XWindowEvent(display, winder, EV_MASK, &event);
	need_empty_loops+=5;
...
X11_locator(int *wx, int *wy)
{
	...
	*wy = (int)vdevice.sizeSy - *wy;
	mask=((mask >> 8) & 0x0f);
	/* VOGL waits for the button to be released... */
	if (oldmask!=0 && mask==0) need_empty_loops+=1;
...
X11_mapcolor(int i, int r, int g, int b)
{
	...
	if (vdevice.depth == 1) {
		...
		if (privatecmap) {
			XStoreColor(display, colormap, &tmp);
		} else if ((stat = XAllocColor(display, colormap, &tmp)) == 0) {
