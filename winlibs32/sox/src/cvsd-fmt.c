...
static size_t myread(sox_format_t * ft, sox_sample_t * buf, size_t len)
{
  priv_t *p = (priv_t *) ft->priv;
  size_t i;
...
static size_t mywrite(sox_format_t * ft, sox_sample_t const * buf, size_t len)
{
  priv_t *p = (priv_t *) ft->priv;
  size_t i;
...
LSX_FORMAT_HANDLER(cvu)
{
  ...
  static sox_format_handler_t const handler = {SOX_LIB_VERSION_CODE,
    ...
    names, SOX_FILE_MONO, start, myread, NULL, start, mywrite, NULL,
