/*
 * Copyright (C) 2000,2002,2003,2007 Bernd Feige
 * 
...
# endif
 ...
 for (channel=0; channel<nr_of_channels; channel++) {
  ...
  printf("\nChannel number %d (Offset within record: %d):\n", channel+1, total_samples_per_record*sizeof(short));
 ...
 printf("Total samples per record: %d -> Size of 1 record %d bytes.\n", total_samples_per_record, total_samples_per_record*sizeof(short));
 ...
 printf("File size is %ld, expected for %ld records is %ld\n", statbuff.st_size, nr_of_records, nr_of_records*total_samples_per_record*sizeof(short)+bytes_in_header);
