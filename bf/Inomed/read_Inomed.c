...
read_Inomed_init(transform_info_ptr tinfo) {
 ...
 local_arg->NumSamples = (statbuff.st_size-local_arg->headersize)/local_arg->bytes_per_point;
...
read_Inomed(transform_info_ptr tinfo) {
 ...
 if (args[ARGS_TRIGTRANSFER].is_set) {
  ...
  read_Inomed_reset_triggerbuffer(tinfo);
  for (trigs_in_epoch=1; code=read_Inomed_read_trigger(tinfo, &trigpoint, &thisdescription), 
